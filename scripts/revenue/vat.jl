let
    ...
    df_total_vat = @from i in df begin
        ...
        @where i.STANDARD_REVENUE.name == "Value added taxes (VAT)"
    ...
    df_total_income =@from i in df begin
        @where i.TIME_PERIOD==TIME_PERIOD
        @where i.UNIT_MEASURE.name == "National currency"
        @where i.STANDARD_REVENUE.name == "Taxes on income, profits and capital gains of individuals and corporations"
        @where i.REF_AREA.name == TARGET_NAME
        @select i
        @collect DataFrame
    end
